...
DEFAULT_AVATAR_DIR = Path("/tmp/avatars") if DATABASE_URL else DATABASE_PATH.parent / "avatars"
AVATAR_DIR = Path(os.getenv("AVATAR_DIR", str(DEFAULT_AVATAR_DIR)))
...
class PgConnection:
    ...
    def execute(self, sql: str, params: tuple[Any, ...] = ()) -> PgCursor:
        sql = self._sql(sql)
        if "INSERT INTO room_state" in sql and "ON CONFLICT" not in sql:
            return PgCursor(self.raw.execute(
                """
...
def db() -> Iterator[sqlite3.Connection]:
    if DATABASE_URL:
        ...
        conn = PgConnection(psycopg.connect(DATABASE_URL, row_factory=dict_row))
        try:
            yield conn
            conn.commit()
        except Exception:
...
def avatar_url(filename: str | None) -> str | None:
    if not filename:
        return None
    bundled_filenames = {item[1] for item in PERSONA_AVATARS}
